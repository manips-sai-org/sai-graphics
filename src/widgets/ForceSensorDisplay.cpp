...
namespace SaiGraphics
{
	...
	void ForceSensorDisplay::initializeLines(chai3d::cWorld *chai_world, chai3d::cColorf forceColor, chai3d::cColorf momentColor)
	{
		// initialize display lines
		_display_line_force = new chai3d::cShapeLine();
		_display_line_force->setShowEnabled(false);
		_display_line_force->m_colorPointA = forceColor;
		_display_line_force->m_colorPointB = forceColor;
		...
		_display_line_moment->m_colorPointA = momentColor;
		_display_line_moment->m_colorPointB = momentColor;
		chai_world->addChild(_display_line_moment);
		_display_line_moment->setLineWidth(4.0);

		// initialize scales
		_force_line_scale = 0.007; // before: 0.02
